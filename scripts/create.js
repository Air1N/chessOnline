...
ue.explosion = function(x, y, n, s, p, c, a, d, g) {
    ...
    for (var i = 0; i < n; i++) {
        var size = s * Math.random();
        var explosionbit = new ue.object(x, y, size, size, "explosion");
        explosionbit.body.solid = false;
        explosionbit.color = c;

        var rp = (0.25 + Math.random() * 0.75) * p;
        var rd = (Math.random() * 2 - 1) * d;
        var px = Math.sin(rd + a);
        var py = Math.cos(rd + a);
